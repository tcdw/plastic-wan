...
export class AdminAuth {
  ...
  async createFirstUser(credentials: AdminCredentials, now = new Date()): Promise<string> {
    ...
    const userId = this.#db.transaction(() => {
      ...
      const created = this.#db
        .query("INSERT INTO admin_users(username, password_hash, created_at, updated_at, last_login_at) VALUES (?, ?, ?, ?, ?)")
  ...
  async login(credentials: AdminCredentials, now = new Date(), clientKey = "unknown"): Promise<string> {
    ...
    this.#db.query("UPDATE admin_users SET last_login_at = ?, updated_at = ? WHERE id = ?")
  ...
  #createSession(userId: bigint, now: Date): string {
    ...
    const iso = now.toISOString();
    this.#db
      .query("INSERT INTO admin_sessions(user_id, token_hash, created_at, expires_at, last_seen_at) VALUES (?, ?, ?, ?, ?)")
...
function assertCredentials(credentials: AdminCredentials): void {
  if (typeof credentials.username !== "string" || !USERNAME_PATTERN.test(credentials.username)) {
    throw new AdminAuthError(400, "invalid_username", "Username must be 3-32 characters of letters, digits, dot, underscore, or hyphen");
  }
  if (typeof credentials.password !== "string" || credentials.password.length < MIN_PASSWORD_LENGTH || credentials.password.length > MAX_PASSWORD_LENGTH) {
    throw new AdminAuthError(400, "invalid_password", `Password must be ${MIN_PASSWORD_LENGTH}-${MAX_PASSWORD_LENGTH} characters`);
  }
}
