...
import { AdminQueryError, parseId, parseLimit, type ListQuery, type Page } from "./audit.ts";
...
export function createMemory(db: Database, body: CreateMemoryBody, warningDays: number, now = new Date()): MemoryAdminItem {
  ...
  db.transaction(() => {
    ...
    const existing = db
      .query<{ id: bigint }, [bigint, number]>("SELECT id FROM conversations WHERE chat_id = ? AND message_thread_id = ?")
      .get(chat.id, body.threadId);
    const conversationId = existing?.id
      ?? BigInt(
        db.query("INSERT INTO conversations(chat_id, message_thread_id, created_at, updated_at) VALUES (?, ?, ?, ?)")
          .run(chat.id, body.threadId, timestamp, timestamp)
          .lastInsertRowid,
      );
    db.query("DELETE FROM memories WHERE expires_at <= ?").run(timestamp);
    db.query("INSERT INTO memories(id, conversation_id, content, created_at, expires_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)")
      .run(id, conversationId, body.content, timestamp, expiresAt, timestamp);
  }).immediate();
  return getItem(db, id, timestamp, warningDays, now);
}

export function updateMemory(db: Database, id: string, body: UpdateMemoryBody, warningDays: number, now = new Date()): MemoryAdminItem {
  const timestamp = now.toISOString();
...
function asRecord(value: unknown): Record<string, unknown> {
  if (typeof value !== "object" || value === null) throw new AdminQueryError("invalid_body", "Request body must be a JSON object");
...
function parseTtlSeconds(value: unknown): number | undefined {
  ...
  if (
    ...
    || value < MEMORY_TTL_MIN_SECONDS || value > MEMORY_TTL_MAX_SECONDS
  ) {
    throw new AdminQueryError("invalid_ttl_seconds", `ttl_seconds must be between ${MEMORY_TTL_MIN_SECONDS} and ${MEMORY_TTL_MAX_SECONDS}`);
  }
  return value;
}
