...
const TgsMetadataSchema = Type.Object(
  { ip: Type.Number(), op: Type.Number() },
  { additionalProperties: true },
);
...
export class TelegramMediaClient implements MediaDownloader {
  ...
  async download(fileId: string, destination: string, signal: AbortSignal): Promise<void> {
    ...
    const encodedPath = file.file_path.split("/").map((part) => encodeURIComponent(part)).join("/");
    const response = await fetch(`https://api.telegram.org/file/bot${this.#token}/${encodedPath}`, {
      signal,
      redirect: "error",
    });
    if (!response.ok || response.body === null) throw new Error(`Telegram media download failed with status ${response.status}`);
...
export class MediaService {
  ...
  createReadImageTool(context: InvocationContext, invocationDeadline: number): AgentTool<typeof ReadImageSchema, { cached: boolean }> {
    return {
      ...
      description: "Analyze one Photo, image Document, or Sticker referenced in this invocation. Accepts only an img_ image_ref shown in the current context. These refs are for reading media here and cannot be sent as stickers; to send a sticker, call search_stickers first to obtain a stk_ sticker_ref.",
  ...
  async analyzeStickerForIndex(stickerId: bigint, signal: AbortSignal): Promise<StickerIndexAnalysis> {
    ...
    const media: MediaRow = {
      ...
      mime_type: sticker.format === "video"
        ? "video/webm"
  ...
  async #analyze(
    ...
    if (media.file_size !== null && media.file_size > BigInt(MAX_DOWNLOAD_BYTES)) throw new Error("Telegram media exceeds 20 MB");
    if (scope.kind === "chat" && this.#chatTokenBudgetReached(scope.chatId)) throw new Error("Chat token budget reached");
    await mkdir(this.#config.paths.media_cache, { recursive: true, mode: 0o700 });
    ...
    try {
      ...
      this.#store.db
        .query("INSERT INTO media_analyses(file_unique_id, analysis_version, provider, model, prompt_version, kind, state, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?) ON CONFLICT(file_unique_id, analysis_version) DO UPDATE SET state = 'pending', updated_at = excluded.updated_at")
        .run(
      ...
      const analysis = this.#store.db
        .query<{ id: bigint }, [string, string]>("SELECT id FROM media_analyses WHERE file_unique_id = ? AND analysis_version = ?")
        .get(media.file_unique_id, version);
      ...
      const releaseChat = scope.kind === "chat"
        ? await this.#modelGate.acquire(scope.chatId.toString(), signal)
        : undefined;
      try {
        ...
        try {
          ...
          response = await this.#models.completeSimple(
            this.#model,
            {
              systemPrompt: media.kind === "sticker"
              ...
              ...(media.kind === "sticker"
                ? {
                    tools: [{
                      ...
                      description: "Record the visual description, emotions, actions, and bilingual search tags for this sticker",
        ...
        if (response.stopReason === "error" || response.stopReason === "aborted") throw new Error("Vision model failed");
        ...
        const expiresAt = media.kind === "sticker"
          ? null
          : new Date(Date.now() + IMAGE_CACHE_DAYS * 24 * 60 * 60_000).toISOString();
        this.#store.db
          .query("UPDATE media_analyses SET state = 'success', description = ?, metadata_json = ?, expires_at = ?, updated_at = ? WHERE id = ?")
          .run(
      ...
      if (analysisId !== undefined) {
        this.#store.db
          .query("UPDATE media_analyses SET state = 'error', failure_count = failure_count + 1, updated_at = ? WHERE id = ?")
  ...
  #startToolCall(invocationId: bigint, toolCallId: string, input: unknown): bigint {
    const created = this.#store.db
      .query("INSERT INTO tool_calls(invocation_id, tool_call_id, tool_name, arguments_json, state, side_effect, created_at) VALUES (?, ?, 'read_image', ?, 'pending', 0, ?)")
  ...
  #recordRejectedTool(invocationId: bigint, toolCallId: string, input: unknown, code: string): void {
    const now = new Date().toISOString();
    this.#store.db
      .query("INSERT INTO tool_calls(invocation_id, tool_call_id, tool_name, arguments_json, state, side_effect, error_code, created_at, finished_at) VALUES (?, ?, 'read_image', ?, 'error', 0, ?, ?, ?)")
  ...
  #startVisionCall(scope: AnalysisScope, analysisId: bigint): bigint {
    const created = this.#store.db
      .query("INSERT INTO model_calls(invocation_id, media_analysis_id, role, provider, model, attempt, state, created_at) VALUES (?, ?, ?, ?, ?, 1, 'pending', ?)")
      .run(
  ...
  #finishVisionCall(callId: bigint, scope: AnalysisScope, message: AssistantMessage): void {
    this.#store.transaction(() => {
      ...
      if (scope.kind === "chat") {
        ...
        this.#store.db
          .query("INSERT INTO daily_usage(utc_date, scope, resource, metric, amount, updated_at) VALUES (?, 'system', 'sticker_index', 'vision_tokens', ?, ?) ON CONFLICT(utc_date, scope, resource, metric) DO UPDATE SET amount = amount + excluded.amount, updated_at = excluded.updated_at")
  ...
  #reserveStickerImage(): boolean {
    return this.#store.transaction(() => {
      ...
      this.#store.db
        .query("INSERT INTO daily_usage(utc_date, scope, resource, metric, amount, updated_at) VALUES (?, 'system', 'sticker_index', 'vision_images', 1, ?) ON CONFLICT(utc_date, scope, resource, metric) DO UPDATE SET amount = amount + 1, updated_at = excluded.updated_at")
        .run(date, now);
      return true;
    });
  }
...
async function prepareMediaImage(
  ...
  await runExternal(
    createLottieCommand([inputPath, outputPath, "--frame", String(frame)]),
    false,
    signal,
  );
  return normalizeImage(outputPath, directory);
...
export function createLottieCommand(argumentsList: readonly string[]): string[] {
  ...
  const runner = "import os, runpy, sysconfig; runpy.run_path(os.path.join(sysconfig.get_path('scripts'), 'lottie_convert.py'), run_name='__main__')";
...
async function runExternal(argv: readonly string[], captureOutput: boolean, signal: AbortSignal): Promise<string> {
  ...
  try {
    const output = captureOutput && processHandle.stdout instanceof ReadableStream
...
async function normalizeImage(inputPath: string, directory: string): Promise<NormalizedImage> {
  ...
  if (metadata.format === undefined || !(metadata.format in ALLOWED_IMAGE_FORMATS)) throw new Error("Unsupported image format");
