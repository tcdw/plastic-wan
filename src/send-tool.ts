...
export function createSendTool(environment: SendToolEnvironment): AgentTool<typeof SendInputSchema, { telegramMessageId: string }> {
  return {
    name: "send",
    ...
    execute: async (toolCallId, input, signal) => {
      ...
      try {
        ...
        environment.store.transaction(() => {
          const now = new Date().toISOString();
          environment.store.db
            .query("UPDATE tool_calls SET state = 'success', result_text = ?, duration_ms = ?, finished_at = ? WHERE id = ?")
            .run(`telegram_message_id=${response.message_id}`, BigInt(Math.round(performance.now() - startedAt)), now, pending.toolId);
          environment.store.db
        ...
        const errorCode = error instanceof GrammyError
...
function recordRejectedSend(
  ...
  const now = new Date().toISOString();
  environment.store.db
    .query("INSERT INTO tool_calls(invocation_id, tool_call_id, tool_name, arguments_json, state, side_effect, error_code, created_at, finished_at) VALUES (?, ?, 'send', ?, 'error', 1, ?, ?, ?)")
...
function recordOutgoingMessage(
  ...
  environment.store.db
    .query("INSERT INTO senders(telegram_type, telegram_id, display_name, username, is_bot, updated_at) VALUES ('user', ?, ?, ?, 1, ?) ON CONFLICT(telegram_type, telegram_id) DO UPDATE SET display_name = excluded.display_name, username = excluded.username, is_bot = 1, updated_at = excluded.updated_at")
  ...
  const revision = environment.store.db
    .query("INSERT INTO message_revisions(message_id, revision_no, sender_id, kind, text, reply_to_message_id, created_at, raw_fragment_json) VALUES (?, 1, ?, ?, ?, ?, ?, ?)")
    .run(
      messageId,
      sender.id,
      input.kind,
  ...
  if (input.kind === "sticker" && stickerFileId !== null) {
    ...
    if (sticker !== null) {
      environment.store.db
        .query("INSERT INTO media(revision_id, kind, file_id, file_unique_id, mime_type, telegram_json) VALUES (?, 'sticker', ?, ?, 'image/webp', ?)")
