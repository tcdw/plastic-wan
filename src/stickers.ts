...
const StickerSetResponseSchema = Type.Object(
  {
    name: Type.String(),
    title: Type.String(),
    stickers: Type.Array(Type.Object(
      {
        file_id: Type.String(),
...
export class StickerService {
  ...
  async sync(): Promise<void> {
    ...
    this.#store.transaction(() => {
      ...
      for (const set of configured) {
        this.#store.db
          .query("INSERT INTO sticker_sets(alias, telegram_name, configured, sync_state, updated_at) VALUES (?, ?, 1, 'pending', ?) ON CONFLICT(alias) DO UPDATE SET telegram_name = excluded.telegram_name, configured = 1, sync_state = 'pending', updated_at = excluded.updated_at")
          .run(set.alias, set.name, now);
      }
      this.#store.db.query("UPDATE stickers SET active = 0 WHERE sticker_set_id IN (SELECT id FROM sticker_sets WHERE configured = 0)").run();
      this.#store.db.query("DELETE FROM sticker_search WHERE sticker_id IN (SELECT CAST(s.id AS TEXT) FROM stickers s JOIN sticker_sets ss ON ss.id = s.sticker_set_id WHERE ss.configured = 0)").run();
    });
    for (const set of configured) {
      try {
        ...
        this.#store.db
          .query("UPDATE sticker_sets SET sync_state = 'error', error_code = 'telegram_sync', updated_at = ? WHERE alias = ?")
  ...
  async runOne(now = new Date(), signal = new AbortController().signal): Promise<boolean> {
    ...
    this.#store.db.query("UPDATE stickers SET index_state = 'running', updated_at = ? WHERE id = ?").run(now.toISOString(), sticker.id);
    try {
      ...
      this.#store.transaction(() => {
        ...
        this.#store.db
          .query("UPDATE stickers SET index_state = 'error', failure_count = ?, next_retry_at = ?, updated_at = ? WHERE id = ?")
          .run(
            failureCount,
  ...
  createSearchTool(
    ...
    return {
      ...
      execute: async (toolCallId, input) => {
        ...
        try {
          const setId = input.set === undefined
            ? undefined
            : this.#store.db
  ...
  #applyStickerSet(alias: string, response: Static<typeof StickerSetResponseSchema>): void {
    ...
    this.#store.db
      .query("DELETE FROM sticker_search WHERE sticker_id IN (SELECT CAST(id AS TEXT) FROM stickers WHERE sticker_set_id = ? AND active = 0)")
      .run(set.id);
    this.#store.db
      .query("UPDATE sticker_sets SET title = ?, telegram_name = ?, sync_state = 'success', last_synced_at = ?, error_code = NULL, updated_at = ? WHERE id = ?")
  ...
  #publishIndex(stickerId: bigint, emoji: string | null, analysis: StickerIndexAnalysis): void {
    ...
    this.#store.transaction(() => {
      ...
      this.#store.db.query("INSERT INTO sticker_search(sticker_id, description) VALUES (?, ?)").run(stickerId.toString(), content);
      this.#store.db
        .query("UPDATE stickers SET current_analysis_id = ?, index_state = 'success', failure_count = 0, next_retry_at = NULL, updated_at = ? WHERE id = ?")
  ...
  #startSearchCall(invocationId: bigint, toolCallId: string, input: unknown): bigint {
    const created = this.#store.db
      .query("INSERT INTO tool_calls(invocation_id, tool_call_id, tool_name, arguments_json, state, side_effect, created_at) VALUES (?, ?, 'search_stickers', ?, 'pending', 0, ?)")
  ...
  #finishSearchCall(
    ...
    this.#store.db
      .query("UPDATE tool_calls SET state = ?, result_text = ?, error_code = ?, duration_ms = ?, finished_at = ? WHERE id = ?")
