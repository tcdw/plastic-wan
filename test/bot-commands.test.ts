...
import { BOT_COMMANDS, BotCommandService, parseBotCommand, registerBotCommands, type BotCommandRegistration, type CommandSender } from "../src/bot-commands.ts";
...
async function setup(
  transform: (toml: string) => string = (toml) => toml.replace(
...
describe("bot command service", () => {
  test("pause expires pending buckets, aborts queued invocations and blocks new buckets", async () => {
    ...
    expect(store.db.query<{ paused_at: string }, []>("SELECT paused_at FROM chat_pause").get()?.paused_at).toBe(FIXED_NOW.toISOString());
    expect(store.db.query<{ state: string }, []>("SELECT state FROM buckets").get()?.state).toBe("expired");
    const invocation = store.db.query<{ state: string; completion_reason: string }, []>("SELECT state, completion_reason FROM invocations").get();
  ...
  test("status reports model, thinking effort and today's token usage", async () => {
    ...
    store.db
      .query("INSERT INTO daily_usage(utc_date, scope, resource, metric, amount, updated_at) VALUES (?, 'chat', ?, 'model_tokens', 1234, ?)")
  ...
  test("status ignores token usage from other days and other chats", async () => {
    ...
    store.db
      .query("INSERT INTO daily_usage(utc_date, scope, resource, metric, amount, updated_at) VALUES (?, 'chat', ?, 'model_tokens', 999, ?)")
      .run("2026-08-14", "123456789", FIXED_NOW.toISOString());
    store.db
      .query("INSERT INTO daily_usage(utc_date, scope, resource, metric, amount, updated_at) VALUES (?, 'chat', ?, 'model_tokens', 888, ?)")
  ...
  test("acting admins refresh their display name in the admin list", async () => {
    ...
    const row = store.db.query<{ display_name: string; added_by: string }, []>("SELECT display_name, added_by FROM bot_admins").get();
...
describe("ingestion command interception", () => {
  test("commands are audited but not stored and never create buckets", async () => {
    ...
    const audit = store.db.query<{ allowed: bigint; rejection_reason: string | null }, []>("SELECT allowed, rejection_reason FROM telegram_updates").get();
...
describe("scheduler pause enforcement", () => {
  test("due processing skips a paused chat's collecting bucket", async () => {
    ...
    store.db
      .query("INSERT INTO buckets(conversation_id, state, first_received_at, deadline_at, created_at, updated_at) VALUES ((SELECT id FROM conversations LIMIT 1), 'collecting', ?, ?, ?, ?)")
      .run(now, now, now, now);
    expect(scheduler.processDue(new Date(start.getTime() + 60_000))).toHaveLength(0);
    const bucket = store.db.query<{ state: string }, []>("SELECT state FROM buckets").get();
    expect(bucket?.state).toBe("collecting");
    expect(store.db.query<{ count: bigint }, [bigint]>("SELECT COUNT(*) AS count FROM chat_pause WHERE chat_id = ?").get(chatId)?.count).toBe(1n);
    store.close();
  });
  ...
  test("startup catch-up skips paused chats", async () => {
    ...
    const bucket = store.db.query<{ state: string; error_code: string | null }, []>("SELECT state, error_code FROM buckets").get();
  ...
  test("pauseChat aborts a running invocation", async () => {
    ...
    const waitForState = async (state: string): Promise<void> => {
      for (let attempt = 0; attempt < 100; attempt += 1) {
        if (store.db.query<{ state: string }, []>("SELECT state FROM invocations LIMIT 1").get()?.state === state) return;
        await Bun.sleep(10);
      }
