...
import { createLottieCommand, MediaService, type MediaDownloader } from "../src/media.ts";
...
test("read_image normalizes once and reuses the 30-day description cache", async () => {
  ...
  await sharp({ create: { width: 32, height: 16, channels: 4, background: { r: 255, g: 0, b: 0, alpha: 0.5 } } }).png().toFile(fixturePath);
  ...
  const faux = fauxProvider({
    provider: "vision",
    models: [{ id: "vision-model", reasoning: true, input: ["text", "image"], contextWindow: 128_000, maxTokens: 8_192 }],
  });
  ...
  expect(store.db.query<{ count: bigint }, []>("SELECT COUNT(*) AS count FROM media_analyses WHERE state = 'success'").get()?.count).toBe(1n);
