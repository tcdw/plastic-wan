...
async function fixture(extra = ""): Promise<Fixture> {
  ...
  await writeTestConfig(directory, configPath, `${testConfigToml(directory)}
${extra}`);
...
test("memories persist per conversation, expire by TTL, and purge expired rows", async () => {
  ...
  try {
    ...
    expect(store.db.query<{ count: bigint }, []>("SELECT COUNT(*) AS count FROM memories WHERE content = 'ephemeral'").get()?.count).toBe(1n);
    purgeExpiredData(store.db, loaded.config, new Date("2026-08-15T15:01:01.000Z"));
    expect(store.db.query<{ count: bigint }, []>("SELECT COUNT(*) AS count FROM memories WHERE content = 'ephemeral'").get()?.count).toBe(0n);
  } finally {
    store.close();
  }
});
...
test("add_memory and delete_memory audit tool calls and respect conversation scope", async () => {
  ...
  try {
    ...
    const added = await addTool.execute("call-1", { content: "Alice prefers short replies" }, new AbortController().signal);
    const id = added.details.id;
    expect(id).toMatch(/^mem_[a-f0-9]{32}$/);
    const row = store.db
      .query<{ tool_name: string; state: string; side_effect: bigint; result_text: string; arguments_json: string }, []>(
    ...
    const custom = await addTool.execute("call-2", { content: "short-lived", ttl_seconds: 3_600 }, new AbortController().signal);
    ...
    const audit = store.db
      .query<{ state: string; result_text: string }, []>("SELECT state, result_text FROM tool_calls WHERE tool_call_id = 'call-4'")
      .get();
...
static_dir = ${JSON.stringify(staticDir.replaceAll("\\", "/"))}`);
  ...
  try {
    ...
    store.db.query("UPDATE memories SET created_at = ?, expires_at = ? WHERE id = ?")
    ...
    const auditWrite = await server.handle(
      request("/api/invocations", { method: "PUT", headers, body: "{}" }),
    );
...
function secondConversation(store: SqliteStore, conversationId: bigint): bigint {
  const chat = store.db.query<{ chat_id: bigint }, [bigint]>("SELECT chat_id FROM conversations WHERE id = ?").get(conversationId);
