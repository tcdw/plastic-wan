...
test("lists text-capable models of configured providers", async () => {
  const switcher = await switcherWith((toml) => toml.replace("[providers.vision]", `${IMAGE_ONLY_MODEL}[providers.vision]`));
  const options = switcher.list();
  expect(options.map((option) => `${option.provider}/${option.model}`)).toEqual(["agent/agent-model", "vision/vision-model"]);
...
test("rejects unknown providers, unknown models and image-only models", async () => {
  ...
  expect(() => switcher.switch("agent", "vision-only")).toThrowError("Model agent/vision-only does not accept text input");
